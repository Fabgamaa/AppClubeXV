...
public partial class _Default : System.Web.UI.Page
{
    ...
    protected void btProcurar_Click(object sender, EventArgs e)	{		string filtro = "";		OLT OLT = new OLT();
        ...
        DataSet dsOLT = OLT.RelatorioPercentualMotivo();
    ...
    protected void gvOLT_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            Label lMTV_ORI = (Label)e.Row.FindControl("lbMTV_ORI");
            if (lMTV_ORI.Text == "UNIFI")
            {
                dUNIFI += Convert.ToDouble(lQTD.Text);
            }
            else
            {
                dPOY += Convert.ToDouble(lQTD.Text);
            }
        }
    ...
    private void gvOLT_Total(object sender, GridViewRowEventArgs e)
    {
        ...
        tcTotal.ColumnSpan = 1; // Total é a coluna 1
        tcTotal.Text = "Total UNIFI";
        gvTotal.Cells.Add(tcTotal);

        tcTotal = new TableCell();
        tcTotal.CssClass = "rodape_tabela";
        tcTotal.Text = dUNIFI.ToString();
        gvTotal.Cells.Add(tcTotal);

        tcTotal = new TableCell();
        tcTotal.CssClass = "rodape_tabela";
        tcTotal.Text = (dUNIFI / dQTD * 100).ToString("N2");
        gvTotal.Cells.Add(tcTotal);

        tcTotal = new TableCell();
        tcTotal.CssClass = "rodape_tabela";
        tcTotal.ColumnSpan = 2; //Até o fim da tabela
        tcTotal.Text = "";
        gvTotal.Cells.Add(tcTotal);
        ...
        //Total POY
        tTotal = (Table)this.gvOLT.Controls[0];
        iIndex = tTotal.Rows.GetRowIndex(e.Row);
        gvTotal = new GridViewRow(iIndex, iIndex, DataControlRowType.Separator, DataControlRowState.Normal);

        tcTotal = new TableCell();
        tcTotal.CssClass = "rodape_tabela";
        tcTotal.ColumnSpan = 1; // Total é a coluna 1
        tcTotal.Text = "Total POY";
        gvTotal.Cells.Add(tcTotal);

        tcTotal = new TableCell();
        tcTotal.CssClass = "rodape_tabela";
        tcTotal.Text = dPOY.ToString();
        gvTotal.Cells.Add(tcTotal);

        tcTotal = new TableCell();
        tcTotal.CssClass = "rodape_tabela";
        tcTotal.Text = (dPOY / dQTD * 100).ToString("N2");
        gvTotal.Cells.Add(tcTotal);
